"""イー・アル・カンフー風格闘ゲーム — 完成: リプレイと記録。毎コマの入力を記録して同じ種で再生し、zip_longest で食い違いを探す。ラウンドの初めの状態は deepcopy で控え（練習モード）、対戦の記録は shelve に保存する。体力などの範囲は __set_name__ の記述子で。"""
...
KEY_CODES = {"left": "l", "right": "r", "up": "u", "down": "d", "punch": "p", "kick": "k"}   # 記録を短くするための 1 文字
CODE_KEYS = {code: key for key, code in KEY_CODES.items()}
REPLAYS_FILE = Path(__file__).with_name("replays") if "__file__" in globals() else Path("replays")   # shelve のファイル（拡張子は shelve が付ける） # ←
...
class Bounded:
    """範囲の決まった数の記述子。__set_name__ で「自分がどのクラスの何という属性か」を教わり、実体は _名前 に置く。"""

    def __init__(self, low: float, high: float, cast=int):
        self.low, self.high, self.cast = low, high, cast

    def __set_name__(self, owner: type, name: str) -> None:
        self.name = "_" + name                              # hp → _hp。dataclass のフィールドはこちら
...
class Fighter:
    ...
    hp = Bounded(0, MAX_HP)                                 # 0〜100 に丸める（注釈が無いのでフィールドではなく記述子）
    stun = Bounded(0.0, 99.0, float)                        # 負にならない
    ...
    _stun: float = field(default=0.0, repr=False)           # 食らって動けない残り時間（記述子 stun 越しに読み書き）
    ...
    def update(self, dt: float) -> None:
        """重力と着地、技のコマ送り、硬直の回復。"""
        self.stun = self.stun - dt                          # 記述子が 0 で止める
...
class ScriptBrain(Brain):
    ...
    def __deepcopy__(self, memo: dict) -> "ScriptBrain":
        """ジェネレータは deepcopy できないので、台本は作り直す（控えを取るのはラウンドの初めなので、台本も初めから）。"""
...
@dataclass
class Replay:
    """1 回のプレイの記録。種・難度・何人目からと、毎コマの入力。入力は "lp" のように 1 文字ずつ並べた文字列。"""

    seed: int
    level: str
    start: int
    frames: list[str] = field(default_factory=list)
    checks: list[tuple] = field(default_factory=list)       # 1 秒ごとの状態の要約（再生が同じ道をたどったか確かめる目印）
    label: str = ""                         # ←

    def record(self, keys: set[str]) -> None:
        self.frames.append("".join(sorted(KEY_CODES[k] for k in keys if k in KEY_CODES)))

    def mark(self, digest) -> None:
        """1 秒ごとに状態の要約を残す（再生と突き合わせる目印）。record のあと、更新まで済ませてから呼ぶ。"""
        if len(self.frames) % FPS == 0:
            self.checks.append(digest())

    def __len__(self) -> int:
        return len(self.frames)

    def __iter__(self):
        """毎コマの「押しているキーの集合」。for keys in replay: で再生できる。"""
        for frame in self.frames:
            yield {CODE_KEYS[c] for c in frame}
...
class Game:
    ...
    def __init__(self, seed: int | None = None, level: str = "normal", start: int = 0):
        ...
        self.pilot_rng = random.Random(seed)                # 自動操縦の乱数。敵と分けておかないと、人が操縦したときに敵の乱数までずれる
        self.replay = Replay(seed if seed is not None else 0, level, start)   # 毎コマの入力をここに足していく
        self.checkpoint: "Game | None" = None               # ラウンドの初めの控え（deepcopy）
        self.new_bout()
    ...
    def new_round(self) -> None:
        ...
        self.checkpoint = None                              # 控えは save_checkpoint() が取る
    ...
    def step(self, keys: set[str], brain_keys: set[str] | None = None, dt: float = 1 / FPS) -> None:
        """1 コマ進める。入力を記録し、押して、相手を動かして、更新する。再生も同じ道を通る。"""
        self.replay.record(keys)
        self.press(keys)
        self.control(self.player, keys, dt)
        self.control(self.enemy, self.brain.decide(self, dt) if brain_keys is None else brain_keys, dt)
        self.update(dt)
        self.replay.mark(self.digest)

    def save_checkpoint(self) -> None:
        """今の状態を控える。deepcopy なので、あとで戻しても元の続きに影響しない。"""
        self.checkpoint = None                              # 控えの中に前の控えを入れない
        self.checkpoint = copy.deepcopy(self)

    def rewind(self) -> "Game | None":
        """控えた所へ戻した新しい Game。控えが無ければ None。"""
        if self.checkpoint is None:
            return None
        back = copy.deepcopy(self.checkpoint)
        back.replay = Replay(self.replay.seed, self.replay.level, self.replay.start)
        return back

    def digest(self) -> tuple:
        """状態の要約。再生が同じ道をたどったかを比べるのに使う。"""
        return (round(self.time, 3), self.phase, self.stage, self.round_no, self.wins, self.losses, self.credits,
                round(self.player.x, 3), round(self.player.y, 3), self.player.hp, round(self.enemy.x, 3), self.enemy.hp)
...
def play(game: Game, practice: bool = False) -> Game:
    """端末で遊ぶ。1/FPS 秒ごとに更新して描く。練習モードならラウンドを落とした時点で控えに戻る。"""
    screen = Screen()
    ...
    with RawTerminal() as term:
        ...
        while game.result is None:
            ...
            if game.phase == "fight" and game.checkpoint is None:
                game.save_checkpoint()                      # ラウンドが始まった所を控える
            game.step(keys, dt=dt)
            if practice and game.phase == "ko" and game.last.endswith("lose"):
                back = game.rewind()
                if back is not None:
                    game, held = back, dict.fromkeys(held, 0.0)
            game.draw(screen)
...
def autopilot(game: Game, dt: float) -> set[str]:
    ...
    if game.phase == "continue":
        return {"punch"}                                    # 自動操縦は必ず続ける（step が記録してから press する）
    gap = abs(e.x - p.x)
    game.pilot_timer -= dt
    ...
    if incoming and p.grounded and game.pilot_rng.random() < 0.5:
    ...
    elif e.anim is not None and gap <= (CHAIN_REACH if e.anim.move is CHAIN_SWING else REACH) + 4 and p.grounded and game.pilot_rng.random() < 0.5:   # 相手の技が見えたら半分はよける
        ...
        keys.add("punch" if game.pilot_rng.random() < 0.6 else "kick")
    return keys


def run_auto(game: Game, limit: float = 600.0) -> Game:
    """自動操縦で最後まで（クリアかゲームオーバーか limit 秒）。入力は記録される。"""
    while game.result is None and game.time < limit:
        game.step(autopilot(game, 1 / FPS))
    return game


def replay_game(rep: Replay, on_frame=None) -> Game:
    """記録から同じ試合を作り直す。種が同じなら乱数も同じなので、同じ道をたどるはず。"""
    game = Game(seed=rep.seed, level=rep.level, start=rep.start)
    for keys in rep:
...
def verify(rep: Replay) -> tuple[bool, str]:
    """記録どおりに再生できるか。もう一度再生して、1 秒ごとの目印を zip_longest で突き合わせる。"""
    fresh: list[tuple] = []
    game = Game(seed=rep.seed, level=rep.level, start=rep.start)
    for i, keys in enumerate(rep, 1):
        if game.result is not None:
            break
        game.step(keys)
        if i % FPS == 0:
            fresh.append(game.digest())
    for i, (saved, now) in enumerate(zip_longest(rep.checks, fresh, fillvalue=None), 1):   # 長さが違っても最後まで比べられる（足りない方は None）
        if saved != now:
            return False, f"{i} 秒目で食い違い: {saved} ≠ {now}"
    return True, f"{len(rep.checks)} 個の目印（{rep.seconds:.1f} 秒）が一致"


def save_replay(rep: Replay, path: Path = REPLAYS_FILE) -> str: # ←
    """記録を shelve に保存して、付けた名前を返す。shelve は「辞書のように使えるファイル」。"""
    with shelve.open(str(path)) as db:
        name = f"{len(db) + 1:03d}-{rep.label or rep.level}" # ←
        db[name] = rep                                      # pickle できるものなら何でも入る（Replay は dataclass） # ←
    return name


def load_replays(path: Path = REPLAYS_FILE) -> list[tuple[str, Replay]]: # ←
    """保存した記録の一覧（名前順）。"""
    try:
        with shelve.open(str(path), flag="r") as db: # ←
            return sorted(db.items())
    except Exception:                                       # まだ 1 つも保存していない
        return []
...
def main():
    ...
    parser.add_argument("--save", metavar="ラベル", nargs="?", const="", help="終わったら記録を保存する") # ←
    parser.add_argument("--list", action="store_true", help="保存した記録の一覧を出して終わる") # ←
    parser.add_argument("--replay", metavar="名前", help="保存した記録を再生する（端末に描く）") # ←
    parser.add_argument("--verify", metavar="名前", help="記録どおりに再生できるか確かめて終わる") # ←
    parser.add_argument("--practice", action="store_true", help="ラウンドを落としたら、その初めからやり直す（deepcopy の控えに戻る）")
    ...
    if args.list:                           # ←
        for name, rep in load_replays():
            print(f"{name:20s} {rep.level:6s} {rep.start + 1} 人目から  {len(rep):5d} コマ（{rep.seconds:.1f} 秒）  種 {rep.seed}")
        return

    if args.verify:                         # ←
        found = dict(load_replays()).get(args.verify)
        if found is None:
            raise SystemExit(f"{args.verify} という記録はありません（--list で一覧）")
        ok, message = verify(found)
        print(("一致しました: " if ok else "食い違いました: ") + message)
        return

    if args.replay:                         # ←
        found = dict(load_replays()).get(args.replay)
        if found is None:
            raise SystemExit(f"{args.replay} という記録はありません（--list で一覧）")
        ...
        with RawTerminal():
            def show(game: Game) -> None:
                game.draw(screen)
                sys.stdout.write("\x1b[H" + screen.render() + game.status() + "   リプレイ\x1b[K\n")
                sys.stdout.flush()
                time.sleep(1 / FPS)
            game = replay_game(found, show)
        print(f"\n{RESULT_TEXT.get(game.result, '記録の終わり')}")
        print("\n".join(game.summary()))
        return
    ...
    if (warning := check_terminal()) is not None:
        raise SystemExit(warning)
    game = Game(seed=args.seed if args.seed is not None else random.randrange(10 ** 6), level=args.level, start=args.start - 1)
    if args.auto:
        screen = Screen()
        with RawTerminal():
        ...
        if args.save is not None:
            game.replay.label = args.save or "demo"
            print(f"記録を {save_replay(game.replay)} に保存しました")
        return
    game = play(game, practice=args.practice)
    if args.save is not None:
        game.replay.label = args.save       # ←
        print(f"記録を {save_replay(game.replay)} に保存しました（--list で一覧、--replay 名前 で再生）") # ←

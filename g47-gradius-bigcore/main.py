"""グラディウス風シューティング — 完成: ビッグコア。ステージの終わりで足止めされ、4 枚のバリアの奥のコアを撃つ。バリアは enum.Flag、揺れと弾は random.gauss、撃破の演出のあと抜ける。"""
...
def main():
    ...
    parser.add_argument("--boss", action="store_true", help="いきなりボスの手前から始める（練習用）")
    ...
    game = Game(seed=args.seed, boss=args.boss)
    if args.auto:
        screen = Screen()
        sys.stdout.write("\x1b[2J\x1b[?25l")
        try:

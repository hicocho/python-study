"""ギャラガ風シューティング — 完成: ステージと演出。全滅させると次のステージ、3 の倍数はチャレンジングステージ。スコア表は CSV。"""
...
def record_score(game: Game, path: Path) -> None:
    """スコア表（CSV）に入れる。表に入らなければ何もしない。"""
    board = ScoreBoard.from_csv(path.read_text(encoding="utf-8")) if path.exists() else ScoreBoard()
    row = ScoreRow("YOU", game.score, game.stage, date.today().isoformat())
    if game.score <= 0 or (len(board.rows) >= SCORE_ROWS and game.score <= board.rows[-1].score):
        return
    row.name = (input(f"スコア表に入ります（{game.score} 点）。名前（8 文字まで、空なら YOU）: ").strip() or "YOU")[:8]
    rank = board.add(row)
    path.write_text(board.to_csv(), encoding="utf-8")
    print(f"{rank} 位に入りました。{path} に書きました。")
    print(board.table())
...
def main():
    ...
    parser.add_argument("--scores", action="store_true", help="スコア表を出して終わる")
    parser.add_argument("--scores-file", type=Path, default=Path(__file__).with_name("scores.csv"), help="スコア表の CSV")
    args = parser.parse_args()

    if args.scores:
        board = ScoreBoard.from_csv(args.scores_file.read_text(encoding="utf-8")) if args.scores_file.exists() else ScoreBoard()
        print(board.table() if board.rows else "まだスコアがありません。")
        return
    ...
    play(game)
    record_score(game, args.scores_file)

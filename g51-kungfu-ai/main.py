"""イー・アル・カンフー風格闘ゲーム — 完成: 最初の敵ワンと AI。設定は ChainMap（既定 → 敵ごと → 難度）、行動は候補に点を付けて dataclass(order=True) の max で選ぶ。人の技に反応の遅れをもって よける。腕前は statistics で測る。"""
...
ENEMIES = {
    "WANG": ChainMap({"kick_rate": 0.5}, DEFAULT_BRAIN),   # ワン。基本形。キックが少し多い
}
LEVELS = {                                  # ←
...
ALL_SPRITES = [OOLONG_STAND, OOLONG_CROUCH, OOLONG_JUMP, OOLONG_PUNCH, OOLONG_KICK, OOLONG_CROUCH_PUNCH, OOLONG_JUMP_KICK, recolored(OOLONG_STAND, tuple(sorted(ENEMY_COLORS.items()))), flipped(OOLONG_PUNCH)]
...
class Game:
    ...
    def __init__(self, seed: int | None = None, enemy: str = "WANG", level: str = "normal"): # ←
        ...
        self.enemy = Fighter(enemy, 92.0, "left", colors=ENEMY_COLORS)
        self.brain = Brain(ChainMap(LEVELS[level], *ENEMIES[enemy].maps), self.rng)   # 難度 → 敵ごと → 既定 の 3 層。手前から順に引く # ←
        self.level = level
    ...
    def resolve(self, attacker: Fighter, victim: Fighter) -> None:
        """攻撃ボックスが相手の体に重なったら当たり。1 つの技で 1 回だけ。"""
...
def autopilot(game: Game, dt: float) -> set[str]:
    ...
    if p.anim is not None:
        return keys
    if e.anim is not None and gap <= REACH + 4 and p.grounded and game.rng.random() < 0.5:   # 相手の技が見えたら半分はよける
...
def bench(n: int, enemy: str = "WANG", level: str = "normal", seed: int = 0) -> dict:
    """自動操縦の自機を n 回 AI と戦わせて、腕前を数字で。勝率・時間の平均と中央値・残り体力のばらつき。"""
    results, times, hp_left, taken = [], [], [], []
    for i in range(n):
        g = Game(seed=seed + i, enemy=enemy, level=level)
        while g.result is None and g.time < 90:
            g.control(g.player, autopilot(g, 1 / FPS), 1 / FPS)
            g.control(g.enemy, g.brain.decide(g, 1 / FPS), 1 / FPS)
            g.update(1 / FPS)
        results.append(g.result == "win")
        times.append(g.time)
        hp_left.append(g.player.hp)
        taken.append(g.enemy.hits)
    return {"n": n, "wins": sum(results), "win_rate": statistics.mean(results), "time_mean": statistics.mean(times), "time_median": statistics.median(times),
            "hp_mean": statistics.mean(hp_left), "hp_stdev": statistics.pstdev(hp_left), "taken_mean": statistics.mean(taken)}
...
def main():
    ...
    if args.bench:
        for level in (LEVELS if args.level == "normal" and args.bench >= 10 else [args.level]): # ←
            ...
            print(f"{level:6s} 勝ち {r['wins']:2d}/{r['n']} ({r['win_rate']:.0%})  時間 平均 {r['time_mean']:.1f} 秒 中央 {r['time_median']:.1f} 秒  残り体力 平均 {r['hp_mean']:.0f} ±{r['hp_stdev']:.0f}  食らった技 {r['taken_mean']:.1f}")
        return

    if (warning := check_terminal()) is not None:
        raise SystemExit(warning)
    game = Game(seed=args.seed, level=args.level) # ←
    if args.auto:
        screen = Screen()
        sys.stdout.write("\x1b[2J\x1b[?25l")
        try:

"""グラディウス風シューティング — 完成: ステージと難度。ステージの表を TOML で持ち、ボスを倒すと次のステージへ。周回とパワーアップで難度が上がる。ハイスコアは JSON、記録は logging、コマンドは argparse のサブコマンド。"""
...
SCORES_FILE = Path(__file__).with_name("scores.json") if "__file__" in globals() else Path("scores.json")
SCORES_KEEP = 10
...
def read_keys(fd: int) -> list[str]:
    ...
    while select.select([fd], [], [], 0)[0]:
        ...
        for token, name in (("\x1b[D", "left"), ("\x1b[C", "right"), ("\x1b[A", "up"), ("\x1b[B", "down"), (" ", "fire"), ("q", "quit"),
                            ("a", "left"), ("d", "right"), ("w", "up"), ("s", "down"), ("x", "power"), ("\r", "power")):
...
def play(game: Game) -> None:
    ...
    try:
        ...
        while game.result is None:
            ...
            game.update(dt)
            game.draw(screen)
            sys.stdout.write("\x1b[H" + screen.render() + game.status() + "   矢印 移動  スペース 撃つ  x 発動  q やめる\x1b[K\n")
            sys.stdout.flush()
...
def load_scores(path: Path = SCORES_FILE) -> list[dict]:
    """ハイスコアの一覧（高い順）。無ければ空。壊れていれば空にして警告。"""
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
        return sorted((d for d in data if isinstance(d, dict) and "score" in d), key=lambda d: -d["score"])
    except FileNotFoundError:
        return []
    except (json.JSONDecodeError, TypeError) as e:
        log.warning("scores.json が読めません（%s）。作り直します", e)
        return []


def save_score(score: int, stage: int, path: Path = SCORES_FILE) -> int | None:
    """ハイスコアに加えて上位 SCORES_KEEP 件だけ残す。入ったら順位（1 から）、入らなければ None。"""
    scores = load_scores(path)
    entry = {"score": score, "stage": stage, "date": date.today().isoformat()}
    scores = sorted([*scores, entry], key=lambda d: -d["score"])[:SCORES_KEEP]
...
def cmd_sheet(args) -> None:                # ←
    args.file.write_bytes(png_bytes(sprite_sheet(ALL_SPRITES), args.scale, background=(16, 16, 32)))
    print(f"{args.file} に {len(ALL_SPRITES)} 枚（{args.scale} 倍）を書き出しました。")
...
def cmd_stages(args) -> None:               # ←
    """ステージの表を出す。--stages で差し替えた表の確認にも。"""
    for i, st in enumerate(args.stages, 1):
        enemies = " ".join(f"{k.name}={w}" for k, w in st.enemies.items())
        print(f"{i}. {st.name:6s} 長さ {st.length:5d}  速さ ×{st.scroll:.2f}  通り道 {st.gap[0]:.0f}→{st.gap[1]:.0f}  ボス HP {st.boss_hp:3d}  敵 {enemies}")


def cmd_script(args) -> None:
    rng = random.Random(args.seed)
    stage = args.stages[args.stage - 1]     # ←
    script = make_script(rng, stage.length, stage.enemies)
    for sp in script:
...
def cmd_map(args) -> None:
    stage = args.stages[args.stage - 1]
    terrain = Terrain(random.Random(args.seed), stage.length, stage.gap)
...
def cmd_scores(args) -> None:
    ...
    for i, d in enumerate(scores, 1):
        print(f"{i:2d}. {d['score']:6d} 点  ステージ {d.get('stage', '?')}  {d.get('date', '')}")


def cmd_play(args) -> None:                 # ←
    """遊ぶ（既定）。--demo なら自動操縦。終わったらハイスコアに記録。"""
    if (warning := check_terminal()) is not None:
        raise SystemExit(warning)
    game = Game(seed=args.seed, boss=args.boss, stages=args.stages, loops=args.loops)
    if args.demo:
        screen = Screen()
        sys.stdout.write("\x1b[2J\x1b[?25l")
        try:
        ...
        play(game)
    print(f"\n{RESULT_TEXT[game.result]}  {game.status()}")
    if game.result != "quit" and not args.demo:
        rank = save_score(game.score, game.stage_number)
        print(f"ハイスコア {rank} 位に入りました！" if rank else "ハイスコアには届きませんでした。")


def main():
    parser = argparse.ArgumentParser(description="グラディウス風シューティング（ステージと難度）")
    parser.add_argument("-v", "--verbose", action="store_true", help="記録を細かく（DEBUG）")
    parser.add_argument("--log", type=Path, metavar="FILE", help="記録をこのファイルに書く（画面を壊さないように、端末には出さない）")
    parser.add_argument("--stages", type=Path, metavar="FILE.toml", help="ステージの表を差し替える") # ←
    sub = parser.add_subparsers(dest="command", metavar="コマンド") # ←

    p = sub.add_parser("play", help="遊ぶ（既定）") # ←
    p.add_argument("--seed", type=int, help="地形の種")
    p.add_argument("--boss", action="store_true", help="いきなりボスの手前から始める（練習用）")
    p.add_argument("--loops", type=int, default=None, help="何周で終わるか（既定は終わらない）")
    p.add_argument("--demo", action="store_true", help="自動操縦で見る")
    p.set_defaults(func=cmd_play)           # ←
    p = sub.add_parser("stages", help="ステージの表を出す")
    p.set_defaults(func=cmd_stages)
    p = sub.add_parser("scores", help="ハイスコアを出す")
    p.set_defaults(func=cmd_scores)
    p = sub.add_parser("map", help="地形の断面を文字で出す")
    p.add_argument("--seed", type=int)
    p.add_argument("--stage", type=int, default=1, help="何面目か（1 から）")
    p.set_defaults(func=cmd_map)
    p = sub.add_parser("script", help="敵の出現表を出す")
    p.add_argument("--seed", type=int)
    p.add_argument("--stage", type=int, default=1)
    p.set_defaults(func=cmd_script)
    p = sub.add_parser("sheet", help="スプライトを 1 枚の PNG に書き出す")
    p.add_argument("file", type=Path, metavar="FILE.png")
    p.add_argument("--scale", type=int, default=4, help="拡大倍率")
    p.set_defaults(func=cmd_sheet)
    p = sub.add_parser("show", help="スプライトを端末に描く")
    p.set_defaults(func=cmd_show)

    args = parser.parse_args(sys.argv[1:] or ["play"])   # サブコマンド無しなら play # ←
    if args.log or args.verbose:
        logging.basicConfig(filename=args.log or "gradius.log", level=logging.DEBUG if args.verbose else logging.INFO,
                            format="%(asctime)s %(levelname)s %(message)s", encoding="utf-8")
    try:
        args.stages = load_stages(args.stages.read_text(encoding="utf-8")) if args.stages else STAGES # ←
    except (OSError, tomllib.TOMLDecodeError, KeyError, ValueError) as e:
        raise SystemExit(f"ステージの表が読めません: {e}")
    args.func(args)                         # ←

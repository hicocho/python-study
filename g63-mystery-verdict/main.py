"""推理ゲーム風 — 完成: 証言の矛盾と告発。食い違う 2 つの証言を突きつけて言い直させ、最後に犯人・手口・動機を指名して採点する。"""
...
# 家具と建物。(横, 縦, 長方形の並び)。長方形は (x, y, 幅, 高さ, 色の文字)
...
# 台詞に埋める名前。Template が $assistant などを差し替える
...
# 突きつけの表。鍵は「2 つの証言の組」なので frozenset
...
def play(game: Game) -> None:
    """端末で遊ぶ。上に一枚絵、下に文章と選択肢。番号を打って進める。"""
    screen = Screen()
    while game.result is None:
        draw(screen, game)
        sys.stdout.write("\x1b[H\x1b[J" + screen.render())
        print(f"\x1b[0m{game.status()}")
        print("-" * 60)
        for line in game.lines:
            print(line)
        print("-" * 60)
        print("  ".join(f"{i + 1} {name}" for i, name in enumerate(game.options())))
        try:
            answer = input("> ").strip()
        except EOFError:
            break
        if answer in ("q", "quit"):
            break
        if answer.isdigit():
            game.choose(int(answer) - 1)
        else:
            game.say("番号で選んでください。")
    print(f"\x1b[0m捜査を終えました。手がかり {len(game.clues)} / {len(CLUES)}。")


def main():
    parser = argparse.ArgumentParser(description="推理ゲーム風 — 現場と聞き込み")
    parser.add_argument("--auto", action="store_true", help="自動で捜査して、手がかりが全部集まるか見る")
    parser.add_argument("--check", action="store_true", help="事件が詰まないか、手がかりの図で確かめる")
    parser.add_argument("--solve", action="store_true", help="自動で捜査してから、正しい答えで告発してみせる") # ←
    parser.add_argument("--sheet", help="場面の絵を PNG に書き出す")
    args = parser.parse_args()
    game = Game()
    if args.check:
        problems = check_case()
        print("解ける順:", " → ".join(CLUES[c].name for c in TopologicalSorter(clue_graph()).static_order()))
        for i, group in enumerate(layers(), 1):
            print(f"  {i} 層目: " + "、".join(CLUES[c].name for c in group))
        print("見つかった問題:", "なし" if not problems else "")
        for problem in problems:
            print("  -", problem)
        return                                      # ←
    if args.solve:
        for step in autopilot(game):
            print(step)
        print(f"集まった手がかり {len(game.clues)} / {len(CLUES)}   証言 {len(game.words)} 件")
        game.answers = [getattr(TRUTH, f.name) for f in fields(Verdict)]
        game.accuse()
        for line in game.lines:
            print(line)
        return
    if args.auto:
        for step in autopilot(game):
            print(step)
        print(f"集まった手がかり {len(game.clues)} / {len(CLUES)}")
        missing = [CLUES[c].name for c in CLUES if c not in game.clues]
        print("届かなかった手がかり:", "、".join(missing) if missing else "なし")
        return
    if args.sheet:
        screen = Screen()
        rows = []
        for pid in PLACES:
            game.place = pid
            draw(screen, game)
            rows.append(("".join("W" if c else "." for c in row) for row in screen.pixels))
        print(f"{args.sheet} は書き出しに main.py の --sheet を使ってください")
        return
    play(game)

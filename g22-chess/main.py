"""チェス — 完成: 盤と駒の動き。2 人で交互に指す。相手をランダム CPU にもできる。class Game にまとめる。"""
...
class Game:
    ...
    def render(self) -> str:
        ...
        return f"{self.board}\n\n{len(self.history) // 2 + 1} 手目 {turn}の番{last}\n"


def main():
    parser = argparse.ArgumentParser(description="チェス（盤と駒の動き）")
    parser.add_argument("--cpu", choices=["w", "b"], help="この色をランダム CPU に任せる")
    args = parser.parse_args()

    game = Game(cpu=args.cpu)
    print("e2e4 のように指す。成るときは e7e8q。undo で戻す、q でやめる。\n")

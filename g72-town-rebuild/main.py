"""街づくり3 — 完成: 季節が巡り、備えが災害を防ぎ、街の歩みが記録に残る。"""
...
Kind = Literal["road", "house", "farm", "shop", "plant", "well", "park", "fire", "levee"]
...
MONTHS = 36                                         # ここまでにどれだけ育てられるか
GOAL = 60                                           # 目標の人口
KEEP = Decimal("50")                                # 自動プレイが手元に残しておくお金

FIRE = 0.10                                         # 月ごとの火事の起きやすさ
FLOOD = 0.06                                        # 月ごとの洪水の起きやすさ
GUARD = 3                                           # 消防署と堤防が守れる距離（マス）
GUARD_AFTER = 25                                    # 下手な市長が備えを建て始める人口
...
RECORD = Path(__file__).with_name("town.csv")       # 記録は main.py の隣に
...
BUILDS = {
    ...
    "fire": Build("fire", "消防署", Decimal("30"), upkeep=Decimal("0.8")),
    "levee": Build("levee", "堤防", Decimal("15"), upkeep=Decimal("0.2")),
}

ORDER = ("road", "house", "farm", "shop", "plant", "well", "park", "fire", "levee")
...
FIRE_HOUSE = (198, 84, 72)                          # 消防署
FIRE_TOP = (240, 168, 152)
LEVEE_C = (150, 150, 140)
LEVEE_TOP = (196, 196, 184)
GUARDED = (120, 200, 160)                           # 守られている範囲の印
BURNT = (52, 48, 52)                                # 焼け跡
FIRE_C = (240, 148, 60)
...
class Town:
    ...
    turning: object = None                          # 季節の終わりのない列（itertools.cycle）
    log: list = field(default_factory=list)         # 毎月 1 行の記録（csv に書き出す）
    ...
    def refuse(self, spot: tuple[int, int], kind: Kind) -> str | None:
        ...
        if kind == "levee" and not any(self.at(n) & Land.WATER for n in neighbours(spot)):
            return "堤防は水ぎわにしか建てられない"
    ...
    def guarded(self, spot: tuple[int, int], guard: Kind) -> bool:
        """その場所が、その備えに守られているか。近さだけで見る。"""
        return any(kind == guard and far(place, spot) <= GUARD
                   for place, kind in self.tiles.items())
        ...
        for resource in RESOURCES:
            ...
            for spot, kind in sorted(self.tiles.items()):
                if resource not in BUILDS[kind].gives or spot not in number:
                    continue
                left[spot] = self.supply_now(kind)
    ...
    def advance(self) -> dict:
        ...
        **順番が大事**。災害を先に起こすと、その月の税が入る前に建物が消える。
        ...
        self.log.append({"月": self.month, "季節": self.season, "人口": self.people,
                         "財布": f"{self.purse}", "家": len(self.houses),
                         "建物": len(self.tiles), "焼け跡": len(self.burnt),
                         "出来事": self.news})
        return got
    ...
    def disaster(self) -> str:
        ...
        備えがあっても**災害そのものは起きる**。起きたうえで止まる。だから
        「消防署のおかげで助かった」が記録に残る。
        """
        fire, flood, pick_a, pick_b = (self.luck.random() for _ in range(4))
        news = []
        if fire < FIRE:
            burnable = sorted(s for s, k in self.tiles.items() if k not in ("road", "fire"))
            if burnable:
                ...
                if self.guarded(spot, "fire"):      # 備えは「起きない」ではなく「止まる」
                    ...
                    self.burnt.add(spot)
        if flood < FLOOD:
            wet = sorted(s for s, k in self.tiles.items() if k != "levee"
                         and any(self.at(n) & Land.WATER for n in neighbours(s)))
            if wet:
                spot = wet[int(pick_b * len(wet))]
                if self.guarded(spot, "levee"):
                    news.append("堤防が水を止めた")
                else:
...
class Game:
    ...
    def status(self) -> str:
        ...
        return (f"{self.town.month:2d}/{MONTHS} 月 {self.town.season}   "
                f"人 {self.town.people:3d}/{GOAL}   "
...
def building(screen: Screen, kind: Kind, spot: tuple[int, int]) -> None:
    ...
    match kind:
        ...
        case "fire":
            screen.box(px + 1, py + 2, CELL - 2, CELL - 3, FIRE_HOUSE)
            screen.box(px + 2, py + 3, CELL - 4, 2, FIRE_TOP)
        case "levee":
            screen.box(px, py + 2, CELL, CELL - 4, LEVEE_C)
            for i in range(0, CELL, 2):
                screen.plot(px + i, py + 2, LEVEE_TOP)
...
def draw(screen: Screen, town: Town, cursor: tuple[int, int] | None = None,
    ...
    for spot, kind in town.tiles.items():           # 守られている所に小さな点を打つ
        if kind in ("fire", "levee"):
            continue
        guard = "levee" if any(town.at(n) & Land.WATER for n in neighbours(spot)) else "fire"
        if town.guarded(spot, guard):
            screen.plot(spot[0] * CELL + CELL - 2, spot[1] * CELL + CELL - 2, GUARDED)
...
def spare_spots(town: Town, main: int, kind: Kind) -> list:
    ...
    for y in range(ROWS):
        if abs(y - main) <= 1:
            continue
        for x in range(COLS):
            if town.refuse((x, y), kind) is None:
                spots.append((x, y))
    return spots
    ...
    while not town.done and not town.over:
        ...
        if town.people >= GUARD_AFTER:              # **備えは、街が立ってから**
            for kind in ("fire", "levee"):          # 早すぎると、家に回すお金が無くなる
                if any(k == kind for k in town.tiles.values()):
                    continue                        # 焼けたら建て直す（無いときだけ建てる）
                spots = spare_spots(town, main, kind)   # 道ぎわは家に取っておく
                if spots and town.purse >= BUILDS[kind].cost + KEEP:
...
def replay_check(count: int = 4) -> list[str]:        # ←
    ...
    for seed in range(1, count + 1):
        first, second = autoplay(seed), autoplay(seed)
        a = [(r["月"], r["出来事"]) for r in first.log if r["出来事"]]
        b = [(r["月"], r["出来事"]) for r in second.log if r["出来事"]]
        same = a == b
        out.append(f"種 {seed}  出来事 {len(a):2d} 件  2 回目と同じ: "
                   f"{'はい' if same else 'いいえ'}  "
                   + "  ".join(f"{month}月 {news}" for month, news in a[:3]))
    return out
...
def check_seeds(count: int = 8) -> list[str]:
    ...
    for seed in range(1, count + 1):
        town = autoplay(seed)
        saved = sum(1 for r in town.log if "止めた" in r["出来事"])
        lost = sum(1 for r in town.log if "焼けた" in r["出来事"] or "流された" in r["出来事"])
        out.append(f"種 {seed}  {town.month:2d} 月  人 {town.people:3d}/{GOAL}  "
                   f"家 {len(town.houses):2d}  財布 {town.purse:>8} 円  "
                   f"守った {saved}  失った {lost}  "
                   f"{'達成' if town.done else 'とどかず'}")
    return out


def main():
    parser = argparse.ArgumentParser(description="街づくり3 — 季節と備えと記録")
    parser.add_argument("--seed", type=int, default=1, help="土地の種")
    parser.add_argument("--check", action="store_true", help="配る土地が目標に届くか見る")
    parser.add_argument("--csv", action="store_true", help="自動プレイの記録を csv に書き出す")
    parser.add_argument("--replay", action="store_true", help="同じ種で同じ災害が起きるか見る")
    ...
    if args.auto:
        ...
        return
    if args.csv:
        town = autoplay(args.seed)
        rows = town.save()
        print(f"{RECORD} に {rows} 行書き出した（{town.month} か月ぶん）")
        for line in open(RECORD, encoding="utf-8-sig").read().splitlines()[:4]:
            print("  " + line)
        return
    if args.replay:
        for line in replay_check():
            print(line)
        return

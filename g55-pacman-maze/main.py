"""パックマン風 — 完成: 迷路とパックマン。矢印キーで通路を走り、124 個のエサを全部食べたらクリア。--auto で自動プレイ。"""
...
def main():
    parser = argparse.ArgumentParser(description="パックマン風 — 迷路とパックマン") # ←
    ...
    if problem := check_terminal():
        print(problem)
        return
    play(Game(seed=args.seed), auto=args.auto)      # ←

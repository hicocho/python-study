"""チェスの棋譜 — 完成: SAN で読み書きし、PGN に保存して読み戻す。リプレイ。"""
...
# 駒ごとの「位置の点」。白から見て 8 段目 → 1 段目の順（FEN と同じ向き）。黒は上下を裏返して使う
PST = {
    "P": [0, 0, 0, 0, 0, 0, 0, 0,
...
class Game:
    ...
    @classmethod
    def load(cls, path: Path, **kwargs) -> "Game":
        return cls.from_pgn(path.read_text(encoding="utf-8"), **kwargs)
    ...
    def render(self) -> str:
        ...
        score = textwrap.shorten(self.movetext, width=70, placeholder="…") if self.sans else "（まだ無し）"
        if len(self.movetext) > 70:                             # 長くなったら末尾を見せる
            score = "…" + self.movetext[-69:]
        return (f"{self.board}\n\n{self.board.fullmove} 手目 {turn}の番{last}{check}{again}\n"
                f"棋譜  {score}\n")


def main():
    parser = argparse.ArgumentParser(description="チェス（先読みする CPU）")
    parser.add_argument("--cpu", choices=["w", "b"], default="b", help="CPU が持つ色（既定は黒）。2 人で指すなら --two")
    parser.add_argument("--two", action="store_true", help="2 人で交互に指す")
    parser.add_argument("--time", type=float, default=THINK_SECONDS, help="CPU が考える秒数（既定 1.0）")
    parser.add_argument("--depth", type=int, help="CPU の読みの深さを固定する（時間は見ない）")
    parser.add_argument("--fen", default=START_FEN, help="この局面から始める")
    parser.add_argument("--save", type=Path, metavar="FILE.pgn", help="終わったら（q でも）棋譜をこのファイルに保存する")
    parser.add_argument("--load", type=Path, metavar="FILE.pgn", help="この棋譜の続きから指す")
    parser.add_argument("--replay", type=Path, metavar="FILE.pgn", help="この棋譜を 1 手ずつ再生する（Enter で進む）")
    ...
    options = dict(cpu=None if args.two else args.cpu, seconds=None if args.depth else args.time, depth=args.depth)
    if args.load:
        game = Game.load(args.load, **options)
        print(f"{args.load} の続きから（{len(game.sans)} 手）。\n")
    else:
        game = Game(fen=args.fen, **options)
        game.headers["White"] = "CPU" if game.cpu == WHITE else "Hico"
        game.headers["Black"] = "CPU" if game.cpu == BLACK else "Hico"
    print("e4 や Nf3 のように SAN で指す（e2e4 でも可）。キャスリングは O-O。成るときは e8=Q。pgn で棋譜、undo で戻す、q でやめる。\n")

    while game.result is None:
        print(game.render())
        if game.board.turn == game.cpu:
            print(f"CPU: {game.cpu_move()}   （{game.thought_text()}）\n")
            continue
        text = input("> ").strip()
        if text.lower() == "q":
            game.result = "quit"
            ...
            if move is None:
                print(f"{text!r} は指せません（合法手: {' '.join(san(game.board, m) for m in game.legal_moves)}）。\n")
            else:
                game.play(move)
    ...
    if args.save:
        game.save(args.save)
        print(f"棋譜を {args.save} に保存しました。")


def replay(path: Path) -> None:
    """棋譜を最初から 1 手ずつ。Enter で次へ。"""
    game = Game.load(path)
    print(game.pgn)
    board = game.history[0][0] if game.history else game.board
    print(f"\n{board}\n")
    for i, ((before, move), text) in enumerate(zip(game.history, game.sans)):
        input(f"{i // 2 + 1}{'.' if i % 2 == 0 else '...'} {text}  （Enter で次へ）")
        board = make_move(before, move)
        print(f"\n{board}\n")
    print(RESULT_TEXT.get(game.result, "（続きあり）"))

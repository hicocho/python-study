"""パックマン風 — 完成: ステージと果物。全部食べると次の面へ。面ごとに速さとイジケ時間が変わり、途中に果物が出る。"""
...
STAGE_PAUSE = 1.5                                   # 面をクリアしてから次の面が始まるまで
...
RESULT_TEXT = {
    "over": "ゲームオーバー。{game.level} 面で力尽きました。{game.score} 点。",
    "quit": "やめました。{game.level} 面、{game.score} 点。",
}
...
class Game:
    ...
    def update(self, dt: float, auto: bool = False) -> None:
        ...
        self.touch()
        if self.remaining == 0:
            self.pause = STAGE_PAUSE                # 面をクリア。少し待って次へ
...
def main():
    parser = argparse.ArgumentParser(description="パックマン風 — ステージと果物")
    ...
    if problem := check_terminal():
        print(problem)
        return
    play(Game(seed=args.seed, skill=args.skill, level=args.level), auto=args.auto)

...
# 面。1 行 12 文字、. は空き。上の行が上に置かれる。gimmick は面の仕掛け（None / descend / dark / rainbow / boss）
STAGES = [
    dict(name="はじまり", map="""
rrrrrrrrrrrr
oooooooooooo
...
def autopilot(world: World) -> str | None:
    ...
    coming = (PADDLE_Y - BALL_R - ball.y) / ball.vy if ball.vy > 0 else 9.0
    near = [b for b in world.bullets if -0.12 < (PADDLE_Y - b.y) / b.vy < 1.0
            and abs(b.x + b.vx * (PADDLE_Y - b.y) / b.vy - world.paddle_x) < world.paddle_w / 2 + 12]
    if near:                                        # 弾が来る
        bullet = min(near, key=lambda b: PADDLE_Y - b.y)
        land = bullet.x + bullet.vx * (PADDLE_Y - bullet.y) / bullet.vy
        half = world.paddle_w / 2
        if coming > 0.6:                            # 玉がまだ遠ければ、いまいる側へよける（弾の下を横切らない）
            side = 1 if world.paddle_x >= land else -1
            world.aim(land + side * (half + 8))
        else:                                       # 玉も来る：弾から遠い側の端で受ける
            drop = ball.x + ball.vx * coming
            side = 1 if land < drop else -1
            world.aim(drop + side * (half - 1.5))
        return None
    if ball.vy <= 0:
    ...
    if world.boss and world.boss.alive:
        targets.append(world.boss)
    if targets:
        goal = min(targets, key=lambda b: abs(b.rect(world.time)[0] + b.rect(world.time)[2] / 2 - x))
        gx = goal.rect(world.time)[0] + goal.rect(world.time)[2] / 2
...
def check() -> None:
    ...
    for spec in STAGES:
        ...
        assert sum(1 for b in bricks if b.breakable) > 0 or spec.get("gimmick") == "boss", f"{name} に壊せるブロックが無い"
        assert spec.get("gimmick") in (None, "descend", "dark", "rainbow", "boss"), f"{name} の仕掛け"
    kinds_used = {ch for spec in STAGES for ch in spec["map"] if ch in KINDS}
    assert kinds_used == set(KINDS), "使っていない種類がある"
    print(f"  {len(STAGES)} 面（" + "、".join(f"{s['name']} {len(parse_stage(s['map']))} 個" for s in STAGES) + f"）、種類は {len(KINDS)} 文字全部使う")
...
""")
    ...
    assert counts["clear"] == len(STAGES) - 1 and counts["end"] == 1 and world.boss is not None and not world.boss.alive
    assert counts["boom"] >= 3 and world.caught >= 5, counts
    print(f"  {world.time:.0f} 秒で {len(STAGES)} 面クリア、落とした {counts['lose']} 回、壊した {world.broken} 個、点 {world.score}、"
          f"最長 {world.best_streak} 連続、爆発 {counts['boom']} 回、拾った {world.caught} 個（悪いの {counts['bad']}）")
    world = World(seed=4)
    counts = play_out(world, limit=3000)
    assert world.over and counts["end"] == 1 and (world.won or counts["lose"] >= LIVES - 1)   # 1UP で増えることがある
...
""")
    ...
    print("● 面の仕掛け")
    index = [s["name"] for s in STAGES].index("降りてくる")
    world = World(seed=1)
    world.started = True
    world.load_stage(index)
    world.ball = Ball(x=60, y=60, vx=0, vy=-40, stuck=False)
    y0 = world.bricks[0].rect(world.time)[1]
    world.update(STEP)
    assert world.bricks[0].rect(world.time)[1] > y0, "降りてくる"
    for brick in world.bricks:
        brick.dy = DESCEND_LIMIT - BRICK_TOP - 4 * BRICK_H + 0.5
    lives = world.lives
    world.ball = Ball(x=60, y=60, vx=0, vy=-40, stuck=False)
    assert until(world) == "lose" and world.lives == lives - 1 and world.bricks[0].dy == 0.0, "限界まで来たら押し戻されて玉を失う"
    index = [s["name"] for s in STAGES].index("七色")
    world = World(seed=1)
    world.started = True
    world.load_stage(index)
    brick = [b for b in world.bricks if b.col == 5 and b.row == 4][0]
    assert brick.left == 3
    world.ball = Ball(x=55, y=BRICK_TOP + 5 * BRICK_H + 3, vx=0, vy=-50, stuck=False)
    assert until(world) == "clank" and brick.left == 2
    world.ball = Ball(x=55, y=BRICK_TOP + 5 * BRICK_H + 3, vx=0, vy=-50, stuck=False)
    assert until(world) == "clank" and brick.left == 1
    world.ball = Ball(x=55, y=BRICK_TOP + 5 * BRICK_H + 3, vx=0, vy=-50, stuck=False)
    assert until(world) == "brick" and brick not in world.bricks and world.score == RAINBOW_POINTS, "七色は 3 回で 30 点"
    index = [s["name"] for s in STAGES].index("暗闇")
    world = World(seed=1)
    world.started = True
    world.load_stage(index)
    world.ball = Ball(x=6, y=BRICK_TOP + 2, vx=0, vy=-40, stuck=False)   # 左上の青の隣
    small = Screen()
    draw(small, world)
    near = small.pixel(5, BRICK_TOP + 2)
    far = small.pixel(115, BRICK_TOP + 2)
    assert sum(near) > sum(far) * 2, "暗闇：玉の近くだけ明るい"
    index = [s["name"] for s in STAGES].index("動く壁")
    world = World(seed=1)
    world.started = True
    world.load_stage(index)
    wall = [b for b in world.bricks if b.kind == "W"]
    assert len(wall) == 3 and not wall[0].breakable
    x0 = wall[0].rect(0.0)[0]
    x1 = wall[0].rect(WALL_PERIOD / 4)[0]
    assert abs(x1 - x0 - WALL_SWAY) < 1e-6, "壁は大きく揺れる"
    wx, wy, ww, wh = wall[1].rect(0.0)
    world.ball = Ball(x=wx + 5, y=wy + wh + 3, vx=0, vy=-50, stuck=False)
    assert until(world) == "clank" and world.ball.vy > 0 and len(wall) == 3, "壁は壊れず跳ね返す"
    print("  降りてくる（限界で押し戻され玉を失う）、七色（3 回で色が変わり 30 点）、暗闇（玉の近くだけ見える）、動く壁（大きく揺れる鉄）")

    print("● ボス")
    index = [s["name"] for s in STAGES].index("ボス")
    world = World(seed=1)
    world.started = True
    world.lives = 3
    world.load_stage(index)
    assert world.boss is not None and world.boss.alive and world.remaining() == 5, "ボスと盾 4 つ"
    bx, by, bw, bh = world.boss.rect(world.time)
    assert bw == BOSS_W
    world.ball = Ball(x=bx + bw / 2, y=by + bh + 3, vx=0, vy=-50, stuck=False)
    got = until(world)
    assert got == "brick" and world.boss.hp == BOSS_HP - 1 and world.ball.vy > 0 and world.boss.width() < BOSS_W, "当てると縮む"
    world.powers["fire"] = 99.0
    world.ball = Ball(x=world.boss.rect(world.time)[0] + 10, y=by + bh + 3, vx=0, vy=-50, stuck=False)
    until(world)
    assert world.boss.hp == BOSS_HP - 3, "火の玉は 2 減る"
    world.powers = {}
    world.bullets = [Bullet(world.paddle_x, PADDLE_Y - 3, 0, BULLET_SPEED)]
    world.balls = [Ball(x=20, y=40, vx=0, vy=-40, stuck=False), Ball(x=40, y=40, vx=0, vy=-40, stuck=False)]
    assert until(world) == "bad" and len(world.balls) == 1 and world.lives == 3, "弾が当たると玉を 1 つ失う"
    world.bullets = [Bullet(world.paddle_x, PADDLE_Y - 3, 0, BULLET_SPEED)]
    assert until(world) == "lose" and world.lives == 2, "最後の 1 つなら落としたのと同じ"
    world.boss.fired_at = -9
    world.ball = Ball(x=20, y=30, vx=0, vy=-40, stuck=False)
    world.update(STEP)
    assert world.bullets, "弾を落とす"
    assert world.boss.interval() == BOSS_FIRE[0] * world.boss.hp / BOSS_HP + BOSS_FIRE[1] * (1 - world.boss.hp / BOSS_HP)
    world.paddle_x = world.target_x = world.boss.rect(world.time)[0] + 5
    world.powers["laser"] = 99.0
    world.balls = [Ball(x=20, y=30, vx=0, vy=-40, stuck=False)]
    hp = world.boss.hp
    assert world.launch() == "laser" and world.boss.hp == hp - 1, "レーザーも効く"
    world.boss.hp = 1
    world.powers = {}
    bx, by, bw, bh = world.boss.rect(world.time)
    world.ball = Ball(x=bx + bw / 2, y=by + bh + 3, vx=0, vy=-50, stuck=False)
    score = world.score
    got = until(world)
    assert not world.boss.alive and world.score >= score + 500, "倒すと +500"
    print(f"  体力 {BOSS_HP}、当てるたびに縮む（{BOSS_W:.0f} → {BOSS_W_MIN:.0f}）、火の玉は 2、レーザーも効く、弾は {BOSS_FIRE[0]} → {BOSS_FIRE[1]} 秒おき、"
          "バーに当たると玉を 1 つ失う、倒すと +500")

"""イー・アル・カンフー風（対戦の流れ）ブラウザ版

CLI 版（g53-kungfu-tournament/main.py）とドット絵・動き・当たり判定はまったく同じ。
定数とパレット、Sprite / sprite()、格闘家の絵、Screen、png_bytes() / data_uri()、Move / Animation / only_when_free / Fighter / Pose / 技の表 / flipped / recolored / DEFAULT_BRAIN / LEVELS / EnemyDef（TypedDict）/ Slot（Generic）/ Projectile / Option / Brain / 台本（yield from）/ ScriptBrain / ENEMIES / Clock / BoutResult / overlaps、
autopilot()、そして class Game を、ステップの目印コメント（# ←）を外しただけで 1 文字も変えずに持ってきている。

持ってこなかったのは端末に描く Screen.render() を使う play() / read_keys() / check_terminal() / RawTerminal / main() だけ。

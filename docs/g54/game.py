"""イー・アル・カンフー風（リプレイと記録）ブラウザ版

CLI 版（g54-kungfu-replay/main.py）とドット絵・動き・当たり判定はまったく同じ。
定数とパレット、Sprite / sprite()、格闘家の絵、Screen、png_bytes() / data_uri()、Move / Animation / only_when_free / Fighter / Pose / 技の表 / flipped / recolored / DEFAULT_BRAIN / LEVELS / EnemyDef（TypedDict）/ Slot（Generic）/ Projectile / Option / Brain / 台本（yield from）/ ScriptBrain / ENEMIES / Bounded（記述子）/ Replay / Clock / BoutResult / replay_game / overlaps、
autopilot()、そして class Game を、ステップの目印コメント（# ←）を外しただけで 1 文字も変えずに持ってきている。

持ってこなかったのは端末に描く play() / read_keys() / check_terminal() / RawTerminal / main() と、ファイルに保存する shelve まわりだけ。
...
replay_button = document.querySelector("#replay-btn")
...
LEVEL = QUERY.get("level", "normal") if QUERY.get("level") in LEVELS else "normal"           # /g53/?level=hard で難度
WANTED_START = QUERY.get("start", "1")
START = int(WANTED_START) - 1 if WANTED_START.isdigit() and 1 <= int(WANTED_START) <= len(ENEMY_ORDER) else 0   # /g53/?start=3 で 3 人目から
...
def draw():
    ...
    time_label.textContent = f"{game.clock:clock}"
    hud.textContent = game.status()
    if playing_back is not None:
        message.textContent = f"リプレイ再生中（{game.replay.seconds:.1f} 秒）"
    if game.result is not None:
        message.textContent = RESULT_TEXT[game.result] + "　" + "　".join(game.summary())


async def loop():
    """1/FPS 秒ごとに更新して描く。CLI 版の play() の while と同じ。ここは dt を 1/FPS に固定する（記録した入力をそのまま再生できるように）。"""
    global running, playing_back
    running = True
    while game.result is None:
        if playing_back is not None:                        # リプレイ: 記録した入力を順に流す
            keys = next(playing_back, None)
            if keys is None:
                playing_back = None
                break
        else:
            keys = autopilot(game, 1 / FPS) if auto else {k for k, down in pressed.items() if down}
        game.step(keys)
        draw()
...
def start(demo: bool = False):
    global game, auto, playing_back
    game = Game(level=LEVEL, start=START)
    auto, playing_back = demo, None
    message.textContent = f"デモ（自機も自動、難度 {LEVEL}）" if demo else f"難度 {LEVEL}　5 人連戦。負けたら何かキーでコンティニュー（3 回）"
...
def on_keydown(event):
    ...
    pressed[key] = True
    if playing_back is not None:                            # リプレイ中に触ったら、そこから自分で続ける
        globals()["playing_back"] = None
    game.press({key})                                       # コンティニュー待ちなら何かキーで続ける
...
def on_replay(event):
    """今の（または終わったばかりの）試合を、記録した入力で最初から再生する。"""
    global game, auto, playing_back
    rep = game.replay
    if not len(rep):
        return
    game = Game(seed=rep.seed, level=rep.level, start=rep.start)
    auto, playing_back = False, iter(rep)
    message.textContent = "リプレイ再生中"
    for button in pad_buttons:

...
CLI 版（g42-kifu/main.py）と駒・盤・手の生成・成り・持ち駒はまったく同じ。
Piece / Square / Move / Board（do / undo / zobrist）、trying() / attacked() / in_check() / legal_moves() / evasions()、Solver / DfPn、evaluate() / Cpu、parse_japanese() / to_kif() / from_kif() / save_json の中身、PROBLEMS、make_move() / describe()、
...
kif_pre = document.querySelector("#kif")
kif_in = document.querySelector("#kif-in")
load_button = document.querySelector("#load-btn")
SAVE_KEY = "g42-save"                                       # localStorage。中身は CLI 版の save.json と同じ
...
def autosave() -> None:
    """CLI 版の save_json() と同じ dict を localStorage に。詰将棋は保存しない。"""
    if game.tsume:
        return
    ...
    window.localStorage.setItem(SAVE_KEY, json.dumps(data))


def restore() -> "Game | None":
    """前回の対局を読み戻す（CLI 版の load_json() と同じ）。無ければ None。"""
    text = window.localStorage.getItem(SAVE_KEY)
    if not text:
        return None
    data = json.loads(text)
    if data.get("result") is not None or not data["moves"]:
        return None
    restored = Game(cpu=data["cpu"], depth=data["depth"])
    for usi in data["moves"]:
        if not restored.play(Move.parse(usi)):
            return None
    return restored


game = restore() or Game(cpu=GOTE, depth=2)
...
def draw():
    ...
    kif_pre.textContent = to_kif(game, "あなた", "CPU") if not game.tsume else "（詰将棋では棋譜を出しません）"
...
@when("click", "#load-btn")
def on_load(event):
    """貼り付けた KIF を並べる（CLI 版の --load と同じ from_kif）。"""
    global game, selected, selected_drop
    try:
        loaded = from_kif(kif_in.value, cpu=GOTE, depth=int(level_select.value))
    except (ValueError, KeyError, IndexError) as e:
        message.textContent = f"読めませんでした: {e}"
        return
    game = loaded
    selected = None
    selected_drop = Piece.NONE
    cpu_button.classList.add("is-on")
    tsume_button.classList.remove("is-on")
    message.textContent = f"{len(game.history)} 手まで並べました"
    draw()
    autosave()
    if cpu_thinking():
        asyncio.ensure_future(cpu_turn())
...
if game.history:
    message.textContent = f"前回の続き（{len(game.history)} 手目まで）"
draw()
if cpu_thinking():
    asyncio.ensure_future(cpu_turn())

...
wind_label = document.querySelector("#wind")
...
def refresh() -> None:
    draw(screen, world)
    screen.flush()
    count_label.textContent = f"{world.in_stage}/{PER_STAGE}"
    ...
    wind_label.textContent = wind_text(world)
    ...
    if world.over:
        message.textContent = (f"おわり。{len(STAGES)} ステージ {ROUND} 枚で命中 {world.hits}、点 {world.score}、最高 {world.best_streak} 連続"

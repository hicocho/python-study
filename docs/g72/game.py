"""街づくり3（季節と備えと記録）ブラウザ版

CLI 版（g72-town-rebuild/main.py）と中身はまったく同じ。季節の巡り（itertools.cycle）も、
備えが災害を止める仕組みも、記録を csv の字にする csv_text も、キーを受ける obey も
1 文字も変えずに持ってきている。

違うのは記録の出し先だけ。CLI は main.py の隣の town.csv、こちらは画面に出す。
...
season_label = document.querySelector("#season")
...
def refresh() -> None:
    ...
    season_label.textContent = game.town.season
    ...
    record_box.textContent = game.town.csv_text() or "（まだ 1 か月も進んでいません）"

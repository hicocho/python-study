...
CLI 版（g39-tsume/main.py）と駒・盤・手の生成・成り・持ち駒はまったく同じ。
Piece / Square / Move / Board（do / undo / zobrist）、trying() / attacked() / in_check() / legal_moves() / evasions()、Solver（詰将棋のソルバー）、PROBLEMS、make_move() / describe()、
...
game = Game(cpu=GOTE, sfen=PROBLEMS[0][1], tsume=PROBLEMS[0][2])
...
def on_hint(event):
    ...
    move = game.hint
    message.textContent = f"次の一手: {describe(game.board, move)}" if move else "この局面からは、手数以内に詰みません。待ったで戻してください"

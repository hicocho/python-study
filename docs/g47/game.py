...
CLI 版（g47-gradius-bigcore/main.py）とドット絵・地形・当たり判定はまったく同じ。
定数とパレット、Sprite / sprite()、スプライト、Screen、png_bytes() / data_uri()、Body / Drawable / Terrain / Starfield / Kind / Spawn / Bullet / Enemy、Barrier（Flag）/ Phase / BigCore / boss_move、FACTORY、make_script()、Power / Capsule / Missile / Laser / Weapon（ABC）と hit()（singledispatch）、Trail（リングバッファ）/ Option / Player / Shot / Explosion、
...
class Game:
    ...
    def __init__(self, seed: int | None = None, boss: bool = False):
        ...
        if boss:                                            # 練習用: いきなりボスの手前から
            self.scroll = STAGE_LENGTH - WIDTH
    ...
    def update(self, dt: float) -> None:
        ...
        if self.boss is None:                               # ボスが出たら足止め
            ...
            if self.scroll >= STAGE_LENGTH - WIDTH:
                self.boss = BigCore(x=WIDTH + 4, y=(HEIGHT - BIGCORE.height) / 2, spr=BIGCORE, kind=Kind.BIGCORE, points=5000, hp=BOSS_HP, mover=boss_move)
        ...
        if self.boss is not None and not self.boss.alive:   # 撃破の演出が終わったら抜ける
    ...
    def status(self) -> str:
        ...
        return f"SCORE {self.score:5d}  残機 {'▲' * max(0, self.lives - 1):<2} 進み {min(100, self.scroll / STAGE_LENGTH * 100):3.0f}%  {gauge}  {self.player.weapon.name}{' +M' if self.player.missile else ''} S{self.player.speed_level}{' O' + str(len(self.player.options)) if self.player.options else ''}{' 盾' + str(self.player.shield) if self.player.shield else ''}{'  CORE ' + '█' * max(0, -(-self.boss.hp // 3)) if self.boss and self.boss.alive else ''}"
...
def autopilot(game: Game, dt: float) -> tuple[int, int]:
    ...
    for t in [*game.enemies, *game.enemy_bullets]:
        if isinstance(t, Bullet):
            ...
            vx, vy = -{Kind.FAN: FAN_SPEED, Kind.GARUN: GARUN_SPEED, Kind.DUCKER: SCROLL_SPEED + DUCKER_SPEED, Kind.CANNON: SCROLL_SPEED, Kind.BIGCORE: 0.0}[t.kind], 0.0
        if t.x <= cx or vx >= 0:
            continue
        seconds = (t.x - cx) / -vx

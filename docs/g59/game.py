"""パックマン風（演出とアトラクトモード）ブラウザ版

CLI 版（g59-pacman-scenes/main.py）と迷路・動き・エサの判定はまったく同じ。
定数と MAZE_TEXT、Maze / Direction、ドット絵と Sprite / Screen / png_bytes()、Walker / Pacman、
Mode と 4 体のおばけ、面の表 Levels、果物、場面 Scene と演出、draw 一式、そして class Game を、
...
ATTRACT_AFTER = 6.0                                 # ゲームオーバーからデモに戻るまで
...
RESULT_TEXT = {
    "quit": "やめました。{game.level} 面、{game.score} 点、最高 {game.best} 点。",
}
...
def draw(screen: Screen, game: "Game") -> None:
    ...
    screen.text(f"{game.level:02d}", 76, 1)                         # 何面目か
    if game.demo:
...
class Game:
    ...
    def status(self) -> str:
        return (f"{self.score:6d} 点   {self.level} 面   残り {self.remaining:3d}   機 {self.lives}   "
                f"{self.scene.value}   最高 {max(self.best, self.score)}")

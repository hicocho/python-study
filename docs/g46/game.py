...
CLI 版（g46-gradius-options/main.py）とドット絵・地形・当たり判定はまったく同じ。
定数とパレット、Sprite / sprite()、スプライト、Screen、png_bytes() / data_uri()、Body / Drawable / Terrain / Starfield / Kind / Spawn / Bullet / Enemy、FACTORY、make_script()、Power / Capsule / Missile / Laser / Weapon（ABC）と hit()（singledispatch）、Trail（リングバッファ）/ Option / Player / Shot / Explosion、
...
ALL_SPRITES = [VIC, BULLET, *FAN, GARUN, DUCKER, CANNON, ENEMY_BULLET, CAPSULE, MISSILE, LASER, *OPTION, SHIELD, *EXPLOSION]
...
class Game:
    ...
    def status(self) -> str:
        ...
        return f"SCORE {self.score:5d}  残機 {'▲' * max(0, self.lives - 1):<2} 進み {min(100, self.scroll / STAGE_LENGTH * 100):3.0f}%  {gauge}  {self.player.weapon.name}{' +M' if self.player.missile else ''} S{self.player.speed_level}{' O' + str(len(self.player.options)) if self.player.options else ''}{' 盾' + str(self.player.shield) if self.player.shield else ''}"

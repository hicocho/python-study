"""イー・アル・カンフー風（技と当たり判定）ブラウザ版

CLI 版（g50-kungfu-moves/main.py）とドット絵・動き・当たり判定はまったく同じ。
定数とパレット、Sprite / sprite()、格闘家の絵、Screen、png_bytes() / data_uri()、Move / Animation / only_when_free / Fighter / Pose / 技の表 / flipped / recolored / overlaps、
...
def sparring(game: Game, dt: float) -> set[str]:
    """相手の練習用の動き。間合い 12〜20 ドットを保ち、ときどき跳ぶ・しゃがむ・技。返すのは押しているキーの集合。"""
    ...
    if gap < 12:
        ...
        keys.add("right" if e.x < p.x else "left")          # 遠ければ寄る（技の届く 18 ドットまで）
    if game.enemy_move:
        keys.add(game.enemy_move)
    if gap < 20 and e.anim is None and game.rng.random() < 0.06:   # 間合いに入っていれば、たまに技
        keys.add(game.rng.choice(["punch", "kick"]))
    return keys
...
def autopilot(game: Game, dt: float) -> set[str]:
    ...
    if p.anim is not None:
        return keys
    if gap > 18:
        keys.add("right" if p.x < e.x else "left")          # 技の届く間合い（18 ドット）まで寄る

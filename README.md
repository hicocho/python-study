...
| g69 | [ぶよぶよ](g69-chain-drop/) | 2 個組の玉を落として同じ色を 4 つつなげて消す落ちものパズル。連鎖で高得点、3 連鎖でフィーバー、お題 10 個、エンドレス、5 色。中身は Python、絵はブラウザで Three.js（映り込み・ブルーム・粒） | `grid[行][列]` と見えない行 / 幅優先のつながり探索 / 連鎖の状態機械（段階ごとの残り時間） / 玉を uid で追う / 全部の置き方を写しの盤で評価する CPU / `MeshPhysicalMaterial` と頂点色の `Points` |

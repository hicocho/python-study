...
| g67 | [モグラ叩き](g67-whack-a-mole/) | 穴から出る 14 種のキャラを叩く。20 秒ずつ 9 ステージで主役が変わり、穴は 3 × 3 → 4 × 3 → 4 × 4 と 3 ステージごとに増え、終盤はふた付き（スライム・おばけ・ウサギ・ネズミの群れ・王様…と、叩いてはいけない爆弾・ハチの巣・サボテン・カメ）。コンボ、反応時間。絵（ドット絵）と動き（穴の状態機械）を分ける | `Sprite` と `recolor` / `Enum` の状態機械と「状態に入った時刻」 / `random.choices(weights=)` / ステージは 1 行の表 / `statistics.mean` / 難しさを時間の式で |
| g68 | [ブロック崩し](g68-breakout/) | バーで玉を跳ね返してブロックを消す。文字で描いた 12 面 ＋ ボス（硬い・鉄・爆発・動くブロック、降りてくる・暗闇・七色・動く壁の仕掛け）、玉 3 つ、連続で倍率、カプセル 11 種（玉 3 つ・広いバー・貫通・磁石・レーザー・火の玉・バリア…）。スマホは画面のどこでも指を左右に動かすだけ（傾きでも） | 文字の地図 → 表で種類 / 軸ごとの当たり判定（x を動かして調べ、次に y）/ 1 ドットずつの歩みですり抜けを防ぐ / 当たった場所で角度 / `math.copysign` / `deviceorientation` でバー |
| g69 | [ぶよぶよ](g69-chain-drop/) | 2 個組の玉を落として同じ色を 4 つつなげて消す落ちものパズル。連鎖で高得点、3 連鎖でフィーバー、お題 10 個、エンドレス、5 色。中身は Python、絵はブラウザで Three.js（目と瞬き・影・映り込み・ブルーム・場所がレベルで変わる） | `grid[行][列]` と見えない行 / 幅優先のつながり探索 / 連鎖の状態機械（段階ごとの残り時間） / 玉を uid で追う / 全部の置き方を写しの盤で評価する CPU / `MeshPhysicalMaterial` と頂点色の `Points` |
| g70 | [アイランド・ダイス](g70-island-dice/) | サイコロを本当に転がして島を進むボードゲーム。自分 ＋ CPU 3 人（性格つき）、分かれ道、船・ワープ・店・じゃんけん・火山、カード 11 種、昼から夜へ変わる島、GSAP の演出と Tone.js の音。中身は Python、物理は Cannon-es、絵は Three.js | マスは表と道の網（`next`）/ 手番の状態機械 / 出目は外から渡す / 地形は式 `height(x, z)` / 性格の表 / Cannon-es（`Body`・`step`・`quaternion.vmult` で上の面） |
...
| g78 | [ワープ・トンネル](g78-space-3d/) | 宇宙船の後ろ視点で迫る小惑星をよける。3D（回転・透視投影・隠面消去・陰影）を標準ライブラリだけで書く | `NamedTuple` にベクトル演算 / 透視投影 `x/z` / 外積で法線 / 画家のアルゴリズム / スキャンラインで塗る |
...
| g81 | [エア・スラローム](g81-flight-rings/) | 峡谷を低空で縫い、輪を順にくぐる 3D フライトレース。コース 3 つ＋種から作るランダム（狭い峡谷・三連橋・S 字・トンネル・滝の上昇）。向きを 3 本のベクトルで持つ 3 軸の回転、内積だけで見るカメラ、高さの表から作る地形、傾く地平線 | ロドリゲスの回転公式 / `Frame(NamedTuple)` の前・上・右 / 双一次補間 / LOD（遠くはまとめて描く） / 面をまたいだ瞬間の補間 |

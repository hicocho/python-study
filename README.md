...
| g70 | [街づくり（土地と道）](g70-town-land/) | 土地に道と建物を置いて街を作る。道でつながった建物が「区画」になる（街づくり 3 段階の 1 つ目） | `enum.Flag`（1 マスに複数の性質）/ `typing.Literal` / `decimal.Decimal`（お金）/ 幅優先で「道でつながった区画」 |
| g71 | [街づくり2（需給と人口と災害）](g71-town-life/) | 月が進むと資源が配られ、満ち足りた家に人が入る。火事と洪水も起きる（街づくり 3 段階の 2 つ目） | `heapq.merge`（並んだ列を混ぜる）/ `math.prod`（どれか 0 なら 0）/ 災害の乱数を分けて引く回数を固定する / `Decimal` の収支 |

...
| g69 | [チェイン・ドロップ](g69-chain-drop/) | 2 個組の玉を落として同じ色を 4 つつなげて消す落ちものパズル。連鎖で高得点、エンドレス、5 色。中身は Python、絵はブラウザで Three.js（つやの球・粒） | `grid[行][列]` と見えない行 / 幅優先のつながり探索 / 連鎖の状態機械（段階ごとの残り時間） / 玉を uid で追う / 全部の置き方を写しの盤で評価する CPU / `MeshPhysicalMaterial` と頂点色の `Points` |

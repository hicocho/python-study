...
| g68 | [ブロック崩し](g68-breakout/) | バーで玉を跳ね返してブロックを消す。文字で描いた 8 面（硬い・鉄・爆発・動くブロック）、玉 3 つ、連続で倍率、カプセルで玉 3 つ・広いバー・貫通・磁石。スマホは画面のどこでも指を左右に動かすだけ（傾きでも） | 文字の地図 → 表で種類 / 軸ごとの当たり判定（x を動かして調べ、次に y）/ 1 ドットずつの歩みですり抜けを防ぐ / 当たった場所で角度 / `math.copysign` / `deviceorientation` でバー |

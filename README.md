...
| g70 | [アイランド・ダイス](g70-island-dice/) | サイコロを本当に転がして島を進むボードゲーム。自分 ＋ CPU 3 人（性格つき）、分かれ道、カード 9 種、全員ゴールで所持金の順位。中身は Python、物理は Cannon-es、絵は Three.js | マスは表と道の網（`next`）/ 手番の状態機械 / 出目は外から渡す / 地形は式 `height(x, z)` / 性格の表 / Cannon-es（`Body`・`step`・`quaternion.vmult` で上の面） |

...
| g69 | [深宇宙シューティング](g69-deep-space/) | 宇宙船で星雲の中を進み、迫る小惑星を撃つ。60 秒のステージ、体力 3・残機 3。中身は Python、絵はブラウザで Three.js（端末は透視投影の丸と点） | x・y・z の世界とカメラ / 透視投影 1 行 / 球の当たり判定 / PyScript から Three.js（`THREE.X.new`、`to_js(dict_converter=)`、`Float32BufferAttribute`） |

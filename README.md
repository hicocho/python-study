...
| g67 | [モグラ叩き](g67-whack-a-mole/) | 3 × 3 の穴から出るモグラを 60 秒で叩く。金・ヘルメット・爆弾、コンボ、反応時間。絵（ドット絵）と動き（穴の状態機械）を分ける | `Sprite` と `recolor` / `Enum` の状態機械と「状態に入った時刻」 / `random.choices(weights=)` / `statistics.mean` / 難しさを時間の式で |

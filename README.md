...
| g81 | [軽飛行機で輪をくぐる](g81-flight-rings/) | 峡谷を低空で縫い、130 m おきの輪 40 個を順にくぐる 3D フライトレース。向きを 3 本のベクトルで持つ 3 軸の回転、内積だけで見るカメラ、高さの表から作る地形、傾く地平線 | ロドリゲスの回転公式 / `Frame(NamedTuple)` の前・上・右 / 双一次補間 / LOD（遠くはまとめて描く） / 面をまたいだ瞬間の補間 |

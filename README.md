...
| g67 | [モグラ叩き](g67-whack-a-mole/) | 穴から出る 14 種のキャラを叩く。25 秒ずつ 6 ステージで主役が変わり、穴は 3 × 3 → 4 × 4、最後はふた付き（スライム・おばけ・ウサギ・ネズミの群れ・王様…と、叩いてはいけない爆弾・ハチの巣・サボテン・カメ）。コンボ、反応時間。絵（ドット絵）と動き（穴の状態機械）を分ける | `Sprite` と `recolor` / `Enum` の状態機械と「状態に入った時刻」 / `random.choices(weights=)` / ステージは 1 行の表 / `statistics.mean` / 難しさを時間の式で |

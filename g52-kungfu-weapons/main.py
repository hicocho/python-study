"""イー・アル・カンフー風格闘ゲーム — 完成: 武器の敵。タオの火の玉、チェンの鎖、ランの手裏剣、飛ぶムー。敵の表は TypedDict、飛び道具の入れ物は Generic[T]、敵の動きは yield from でつないだ台本（ジェネレータ）。"""
...
def main():
    ...
    parser.add_argument("--enemy", choices=ENEMY_ORDER, default="WANG", help="相手（既定 WANG）") # ←
    ...
    if args.bench:
        for level in (LEVELS if args.level == "normal" and args.bench >= 10 else [args.level]):
            r = bench(args.bench, enemy=args.enemy, level=level, seed=args.seed or 0) # ←
            print(f"{args.enemy:5s} {level:6s} 勝ち {r['wins']:2d}/{r['n']} ({r['win_rate']:.0%})  時間 平均 {r['time_mean']:.1f} 秒 中央 {r['time_median']:.1f} 秒  残り体力 平均 {r['hp_mean']:.0f} ±{r['hp_stdev']:.0f}  食らった技 {r['taken_mean']:.1f}")
        return

    if (warning := check_terminal()) is not None:
        raise SystemExit(warning)
    game = Game(seed=args.seed, enemy=args.enemy, level=args.level)
    if args.auto:
        screen = Screen()
        sys.stdout.write("\x1b[2J\x1b[?25l")
        try:

"""将棋 — 完成: 棋譜。日本語の表記で指し、KIF に書いて読み戻し、JSON で途中から再開、zip の棋譜集。将棋 6 段階の完成形。"""
...
# 駒の価値（歩を 100 として）。添字は種類のビット位置（FU=1 → 1、KY=2 → 2、KE=4 → 3、…、OU=128 → 8）
...
KIF_DIR = Path(__file__).with_name("kifu")
...
# 日本語の手。「７六歩」「２二角成」「５五角打」「同歩」「同　歩成」「７六歩(77)」（KIF は元のマスが付く）
MOVE_RE = re.compile(r"""
    ...
    (?:\((?P<src>[1-9]{2})\))?
""", re.VERBOSE)
KANJI_KIND = {"歩": Piece.FU, "香": Piece.KY, "桂": Piece.KE, "銀": Piece.GI, "金": Piece.KI, "角": Piece.KA, "飛": Piece.HI, "玉": Piece.OU, "王": Piece.OU,
              "と": Piece.FU, "杏": Piece.KY, "圭": Piece.KE, "全": Piece.GI, "馬": Piece.KA, "龍": Piece.HI, "竜": Piece.HI,
              "成香": Piece.KY, "成桂": Piece.KE, "成銀": Piece.GI}


def parse_japanese(board: Board, text: str, last: Move | None = None) -> Move | None:
    """「７六歩」のような日本語の手を、今の局面の合法手から探す。見つからなければ None。"""
    text = unicodedata.normalize("NFKC", text.strip())     # 全角の数字・記号を半角に（漢数字はそのまま）
    ...
    if m["dst"] == "同":
        ...
        dst = Square(int(m["dst"][0]), RANK_KANJI_INDEX.get(m["dst"][1], 0) or int(m["dst"][1]))
    kind = KANJI_KIND[m["piece"]]
    promoted_piece = m["piece"][0] == "成" or m["piece"] in "と杏圭全馬龍竜"
    candidates = []
    for move in legal_moves(board):
        if move.dst != dst:
            continue
        if m["drop"] or move.src is None:
            if move.src is None and move.drop == kind and not m["src"]:
                candidates.append(move)
            continue
        piece = board[move.src]
        if kind_of(piece) != kind or (Piece.PROMOTED in piece) != promoted_piece:
            continue
        if m["src"] and move.src != Square(int(m["src"][0]), int(m["src"][1])):
            continue
        if m["promote"] == "成" and not move.promote or m["promote"] == "不成" and move.promote:
            continue
        candidates.append(move)
    if len(candidates) == 2 and candidates[0].dst == candidates[1].dst and candidates[0].src == candidates[1].src:
        candidates = [c for c in candidates if not c.promote] if m["promote"] != "成" else [c for c in candidates if c.promote]
    return candidates[0] if len(candidates) == 1 else None


RANK_KANJI_INDEX = {k: i for i, k in enumerate("一二三四五六七八九", 1)}


def kif_move(board: Board, move: Move, last: Move | None) -> str:
    """KIF の 1 手。「７六歩(77)」「同　歩(33)」「２二角成(88)」「５五角打」。"""
    dst = "同　" if last is not None and move.dst == last.dst else str(move.dst)
    if move.src is None:
        return f"{dst}{KANJI[move.drop]}打"
    piece = board[move.src]
    name = kanji_of(piece)
    if Piece.PROMOTED in piece and kind_of(piece) in (Piece.KY, Piece.KE, Piece.GI):
        name = "成" + KANJI[kind_of(piece)]                     # KIF は 成香・成桂・成銀
    return f"{dst}{name}{'成' if move.promote else ''}({move.src.file}{move.src.rank})"


def to_kif(game: "Game", sente: str = "先手", gote: str = "後手") -> str:
    """対局を KIF 形式の文字列に。"""
    lines = [f"開始日時：{datetime.now():%Y/%m/%d %H:%M:%S}", "手合割：平手", f"先手：{sente}", f"後手：{gote}", "手数----指手---------消費時間--"]
    last = None
    for i, (board, move) in enumerate(game.history, 1):
        lines.append(f"{i:4d} {kif_move(board, move, last)}")
        last = move
    if game.result is not None and game.result != "quit":
        n = len(game.history)
        winner = "先手" if game.result in (SENTE, "sente-stalemate", "sente-perpetual") else "後手"
        if game.result == "repetition":
            lines.append(f"まで{n}手で千日手")
        elif game.result == "tsume-fail":
            lines.append(f"まで{n}手で不詰")
        else:
            lines.append(f"まで{n}手で{winner}の勝ち")
    return "\n".join(lines) + "\n"


def from_kif(text: str, cpu: str | None = None, depth: int = 0) -> "Game":
    """KIF を読んで、その手順を指し直した Game を返す。読めない手があれば ValueError。"""
    game = Game(cpu=cpu, depth=depth)
    for raw in text.splitlines():
        line = unicodedata.normalize("NFKC", raw).strip()
        m = re.match(r"^(\d+)\s+(.+?)(?:\s+\(|$)", line)
        if not m or line.startswith("手数"):
            continue
        body = m.group(2)
        if body.startswith(("投了", "中断", "千日手", "詰み", "持将棋")):
            break
        move = parse_japanese(game.board, body, game.last_move)
        if move is None or not game.play(move):
            raise ValueError(f"{m.group(1)} 手目「{body}」が指せません")
    return game


def save_json(game: "Game", path: Path) -> None:
    """途中の対局を JSON に。手順（USI）と設定だけ保存し、盤は読み戻すときに指し直す。"""
    data = {"moves": [move.usi for _, move in game.history], "cpu": game.cpu, "depth": game.engine.depth if game.engine else 0,
            "result": game.result, "saved": datetime.now().isoformat(timespec="seconds")}
    path.write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")


def load_json(path: Path) -> "Game":
    data = json.loads(path.read_text(encoding="utf-8"))
    game = Game(cpu=data["cpu"], depth=data["depth"])
    for usi in data["moves"]:
        if not game.play(Move.parse(usi)):
            raise ValueError(f"{usi} が指せません")
    return game


def archive_kif(zip_path: Path, name: str, kif: str) -> int: # ←
    """棋譜集（zip）に KIF を 1 つ足す。中の数を返す。"""
    with zipfile.ZipFile(zip_path, "a", compression=zipfile.ZIP_DEFLATED) as zf: # ←
        zf.writestr(name, kif)              # ←
...
def main():
    ...
    parser.add_argument("--cpu", choices=[SENTE, GOTE], help="この側を CPU に任せる（b 先手 / w 後手）")
    parser.add_argument("--depth", type=int, default=CPU_DEPTH, help="CPU の読みの深さ（0 でランダム）")
    parser.add_argument("--time", type=float, metavar="SEC", help="CPU の 1 手の時間。あれば深さは上限になる")
    parser.add_argument("--eval", action="store_true", help="--sfen の局面の評価と、CPU の手を出して終わる")
    parser.add_argument("--profile", action="store_true", help="CPU の 1 手を cProfile で測って、重い関数を出す")
    parser.add_argument("--selfplay", type=int, metavar="N", help="深さ N の CPU と、ランダム CPU を対局させて終わる")
    parser.add_argument("--load", type=Path, metavar="FILE.kif", help="KIF を読んで並べ、続きから指す")
    parser.add_argument("--resume", action="store_true", help="save.json から再開する")
    parser.add_argument("--archive", type=Path, metavar="FILE.zip", help="終わったら KIF をこの棋譜集（zip）に足す") # ←
    parser.add_argument("--list", type=Path, metavar="FILE.zip", help="棋譜集の中身を出して終わる") # ←
    ...
    if args.list:                           # ←
        with zipfile.ZipFile(args.list) as zf:
            for info in zf.infolist():      # ←
                kif = zf.read(info.filename).decode("utf-8")
                print(f"{info.filename}  {info.file_size} バイト  {kif.splitlines()[-1]}")
        return
    board = Board.from_sfen(args.sfen)
    if args.eval or args.profile:
        cpu = Cpu(args.depth, args.time)
        print(board)
        print(f"\n手番から見た駒割り: {evaluate(board):+d}")
        if args.profile:
            profiler = cProfile.Profile()
            move = profiler.runcall(cpu.choose, board.copy())
            stats = pstats.Stats(profiler).sort_stats("cumulative")
            print(f"CPU の手: {describe(board, move)}   {cpu.nodes} 局面\n")
            stats.print_stats(12)
        else:
            start = time.perf_counter()
            move = cpu.choose(board.copy())
            print(f"CPU の手（深さ {args.depth}）: {describe(board, move)}   {cpu.nodes} 局面 {time.perf_counter() - start:.2f} 秒")
        return
    if args.selfplay is not None:
        game = Game(cpu=SENTE, depth=args.selfplay, seed=1)
        weak = Game(cpu=GOTE, depth=0, seed=1)
        while game.result is None and len(game.history) < 300:
            if game.board.turn == SENTE:
                move = game.cpu_move()
            else:
                move = weak.rng.choice(game.legal_moves)
                game.play(move)
            print(f"{len(game.history):3d}. {game.recent[-1]}", end="\n" if len(game.history) % 6 == 0 else "  ")
        print(f"\n{RESULT_TEXT.get(game.result, '300 手で打ち切り')}")
        return
    if args.moves:
    ...
    elif args.resume and SAVE_PATH.exists():
        game = load_json(SAVE_PATH)
        print(f"{SAVE_PATH.name} から再開（{len(game.history)} 手目まで）。\n")
    elif args.load:
        game = from_kif(args.load.read_text(encoding="utf-8"), cpu=args.cpu, depth=args.depth)
        print(f"{args.load.name} を並べました（{len(game.history)} 手）。続きから指せます。\n")
    else:
        game = Game(cpu=args.cpu, sfen=args.sfen, depth=args.depth, seconds=args.time)
        print("７六歩 のように指す（USI の 7g7f でも）。成るときは ２二角成、打つときは ５五角打、直前と同じマスは 同歩。undo で戻す、save で保存、q でやめる。\n")
    ...
    while game.result is None:
        ...
        if game.board.turn == game.cpu:
            ...
            start = time.perf_counter()
            move = game.cpu_move()
            nodes = f"   {game.engine.nodes} 局面 {time.perf_counter() - start:.1f} 秒" if game.engine and not game.tsume else ""
            print(f"CPU: {describe(before, move)}{nodes}\n")
            continue
        text = input("> ").strip()
        if text == "q":
            game.result = "quit"
        elif text == "save" and not game.tsume:
            save_json(game, SAVE_PATH)
            print(f"{SAVE_PATH.name} に保存しました。--resume で再開できます。\n")
        elif text == "kif" and not game.tsume:
            print(to_kif(game))
        elif text == "undo":
            game.undo()
            ...
            move = parse_japanese(game.board, text, game.last_move)
            if move is None:
                try:
                    move = Move.parse(text)
                except (ValueError, IndexError, KeyError):
                    print("７六歩 か 7g7f の形で入力してください。\n")
                    continue
    ...
    if not game.tsume and game.history:
        kif = to_kif(game, "あなた" if game.cpu != SENTE else "CPU", "CPU" if game.cpu == GOTE else "あなた")
        KIF_DIR.mkdir(exist_ok=True)
        name = f"{datetime.now():%Y%m%d-%H%M%S}.kif"
        (KIF_DIR / name).write_text(kif, encoding="utf-8")
        print(f"棋譜を {KIF_DIR.name}/{name} に書きました。")
        if args.archive:                    # ←
            print(f"{args.archive.name} に足しました（{archive_kif(args.archive, name, kif)} 局）。")

...
# 峡谷の道すじの制御点 (x, z)。地図（2560 m 四方）を大きく S 字に横切る。真ん中あたりで急な S 字
...
# 区間（道のり m で区切る）。name は表示、kind で谷の幅・障害物・輪の並びが変わる
...
def check() -> None:
    ...
    assert floor_at(int(5300 / PATH_STEP)) - floor_at(int(4500 / PATH_STEP)) > 120, "滝の上昇で谷底が上る"
    world = World(seed=1)
    world.started = True
    world.clock = 0.0
    world.time = 0.001

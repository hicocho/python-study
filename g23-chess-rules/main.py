"""チェスのルール — 完成: チェック・キャスリング・アンパッサン。合法手だけ指せる。チェックメイトで終わる。"""
...
class Game:
    ...
    def render(self) -> str:
        ...
        return f"{self.board}\n\n{len(self.history) // 2 + 1} 手目 {turn}の番{last}{check}\n"


def main():
    parser = argparse.ArgumentParser(description="チェス（ルール）")
    ...
    game.board = Board.from_fen(args.fen)
